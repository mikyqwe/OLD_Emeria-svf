...
ShadowZoneLIB.Settings = function()
	if (ShadowZoneLIB.data == nil) then
		ShadowZoneLIB.data = {
			...
			--------------------------------------						
			["inside_index"] = 144, -- Dungeon index
			
			["pos_1_floor"] = {16894, 22609}, -- Dungeon coordinations to 1. Floor
			["pos_2_floor"] = {17841, 22987}, -- Dungeon coordinations to 2. Floor
			["pos_3_floor"] = {17127, 22671}, -- Dungeon coordinations to 3. Floor
			["pos_4_floor"] = {17428, 22644}, -- Dungeon coordinations to 4. Floor
			["pos_5_floor"] = {17205, 22927}, -- Dungeon coordinations to 5. Floor
			["pos_6_floor"] = {17752, 22714}, -- Dungeon coordinations to 6. Floor
			["pos_7_floor"] = {17877, 23699}, -- Dungeon coordinations to 7. Floor
			["pos_8_floor"] = {16873, 23787}, -- Dungeon coordinations to 8. Floor
			["pos_9_floor"] = {17273, 23858}, -- Dungeon coordinations to 9. Floor
			
			["outside_index"] = 145, -- Index of maps, where are players teleported from the dungeon (Cities)
			["outside_pos"] = {19082, 22828}, -- Coords of the maps where are players teleported from the dungeon (Cities)	
			...
			["regen_file"] = "data/dungeon/shadow_zone/", -- Path where regens are stored
			...
			["fourth_boss_pos"] = {984, 154},  --- Spawn coordinates of fourth boss (Shadow Azrael - 4385)
			...
			["ABBADON_HP_SMALL"] = 550000, --- This HP will be set for 6th boss (4387) if Shadow amulet (30850) IS  active
			--------------------------------------			
			-- Dungeon settings
			--------------------------------------			
			...
			-- Timers--
			--------------------------------------			
			["time_to_jump"] = 10, --- Time after players will be teleported to next floor
			...
			["dungeon_cooldown"] = 1800, --- Time until players can't enter dungeon afte they finish it
			};
	end
...
ShadowZoneLIB.clearDungeon = function()
...
ShadowZoneLIB.checkEnter = function()
	...
	if party.is_party() then
		...
		if (party.get_near_count() < settings["minimumPartyMembers"]) then
			say(string.format("If you want to enter the Shadow tower,[ENTER]there must be atleast %d players with you...", settings["minimumPartyMembers"]))
			return false;
		end
		...
		if (not minLevCheck) then
			say(string.format("If you want to enter the Shadow tower,[ENTER]every each group member must be level %d.[ENTER][ENTER]The next following players don't have the necessary level:", settings["minimumLevel"]))
...
ShadowZoneLIB.CreateDungeon = function()
	local settings = ShadowZoneLIB.Settings();
...
ShadowZoneLIB.spawnSecondStone = function()
	local settings = ShadowZoneLIB.Settings();
...
ShadowZoneLIB.spawnRandomBoss = function()
	local settings = ShadowZoneLIB.Settings();
	...
	d.set_unique ("RandomBoss", d.spawn_mob(Boss[n], boss_pos[1], boss_pos[2]))
end

ShadowZoneLIB.setReward = function()
	local settings = ShadowZoneLIB.Settings();
...
ShadowZoneLIB.noCoolTimer = function()
	if party.is_party() then
		for _, pid in pairs({party.get_member_pids()}) do
			q.begin_other_pc_block(pid);
			
			cmdchat(string.format("SetDungeonCoolTime %d %d", d.getf("ShadowZone_floor"), d.getf("ShadowZone_nocool_timer")));
